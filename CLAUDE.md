...
Requires `pdflatex` and `pdfunite`. Run from `paper/`:

```bash
./xpower-banq.sh           # Build whitepaper PDF (runs pdflatex twice)
./xpower-banq-apx.sh       # Build appendices PDF
./xpower-banq-all.sh       # Build both + merge into xpower-banq-all.pdf
```
...
- `xpower-banq.tex` — whitepaper
- `xpower-banq-apx.tex` — appendices
