...
% ==============================================================================
\begin{abstract}
We present \textbf{XPower Banq}, a permissionless DeFi lending protocol on the Ethereum Virtual Machine. The protocol introduces: (1)~\emph{optionally locked positions} that attenuate liquidation cascades by factor~$(1{-}\phi)$, (2)~\emph{lethargic governance} with time-weighted parameter transitions bounded by $0.5\times$--$2\times$ per cycle, (3)~\emph{beta-distributed position caps} with holder-count scaling that rate-limits capacity accumulation to $O(\sqrt{k})$ for $k$~accounts, (4)~\emph{transferable debt positions} with inverted ERC20 semantics enabling capital-efficient debt assumption liquidation, and (5)~\emph{log-space TWAP oracles} with bidirectional geometric mean spread computation and logarithmic spread scaling. The protocol prioritizes stability over capital efficiency: default parameters yield 33\% LTV with a 200\% over-collateralization buffer, adjustable via governance. We provide technical analysis of the architecture, security properties, simulation results, and limitations.
\end{abstract}

\textbf{Keywords:} DeFi, Lending Protocol, Liquidation, Oracle, TWAP, Governance, Interest Rates
...
The protocol comprises five core contracts, depicted in Figure~\ref{fig:architecture}:
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Acma}: An access control wrapper around OpenZeppelin's \texttt{AccessManager}~\cite{openzeppelin2020}.
\end{enumerate}
...
For each user $u$ and position type $p \in \{\text{supply}, \text{borrow}\}$, the protocol maintains $\text{balance}(u,p)$, $\text{lock}(u,p)$, and $\text{liquid}(u,p) = \text{balance} - \text{lock}$. Operations accept a \texttt{lock} flag; when true, supplied assets cannot be redeemed and borrowed amounts cannot be settled. Transfers proportionally move locked amounts:
\begin{equation}
...
\textbf{Irrevocability.} Locks are permanent---there is no unlock mechanism. A user can exit only via transfer (which proportionally moves the lock) or liquidation. Crucially, only the \emph{principal} is locked: accrued interest on locked supply positions can be redeemed at any time, and accrued interest on locked borrow positions can be settled at any time, too. Still, the design creates a fundamental tension: cascade protection depends on widespread adoption, yet rational adoption requires holding periods exceeding 5~years at typical utilization \cite[\S C]{apx}. The protocol addresses this tension by tying APY incentives to utilization levels.
...
Because locked supply tokens cannot be redeemed, liquidation transfers position tokens without underlying assets leaving the vault. Only the unlocked fraction $(1{-}\phi)$ generates sell pressure. A full proof appears in \cite[\S B]{apx}. Simulation confirms that at a 25\% price shock, unlocked positions suffer 85.7\% liquidation versus 29.4\% when fully locked \cite[\S E]{apx}.
...
\end{align}
where $r_t = \frac{1}{2}(\log_2(1+s^{AB}_t) + \log_2(1+s^{BA}_t))$. Smoothing in $\log_2$ space produces geometric mean temporal averaging (as in Uniswap~V3~\cite{uniswap2021}). With $\alpha = 0.944$ (12-period half-life) and hourly refreshes, approximately 40~hours of sustained manipulation achieves 90\% price deviation.
...
The protocol maintains a symmetric spread: $R_{\text{borrow}} = R_{\text{base}} \cdot (1{+}s)$, $R_{\text{supply}} = R_{\text{base}} \cdot (1{-}s)$, generating $2s$ margin on interest flows. See \cite[\S A]{apx} for mathematical foundations including interest accrual and time-weighted integration.
...
Full proofs of the above properties are provided in \cite[\S B]{apx}.
...
\textbf{Simulation Limitations.} The simulation uses 1,000 agents, which is too few for high statistical confidence. Health factor distributions are assumed rather than calibrated against real DeFi data. The linear price impact model~\cite{kyle1985continuous} is a first-order approximation; real order books exhibit concave impact. No confidence intervals or sensitivity analysis on $k$ are provided, and historical stress events are discussed narratively but not backtested. Implementation details appear in \cite[\S E]{apx}.

\subsection{Gas Cost Analysis}

\begin{table}[htbp]
\centering
\caption{Cross-protocol gas comparison, warm interactions (kgas)}
\label{tab:gas-comparison}
\small
\begin{tabularx}{0.95\columnwidth}{@{}X rrrrr@{}}
\toprule
\shortstack[l]{\vphantom{Xp}Operation\\\vphantom{Xp}} & \shortstack[r]{\vphantom{Xp}Morpho\\\vphantom{Xp}Blue} & \shortstack[r]{\vphantom{Xp}Comp.\\\vphantom{Xp}V2} & \shortstack[r]{\vphantom{Xp}Aave\\\vphantom{Xp}V3} & \shortstack[r]{\vphantom{Xp}Aave\\\vphantom{Xp}V4} & \shortstack[r]{\vphantom{Xp}XPower\\\vphantom{Xp}Banq} \\
\midrule
...
XPower Banq presents a DeFi lending protocol that prioritizes stability over yield optimization. Its strongest contributions are lethargic governance---a genuine innovation with formal rate bounds---and the log-space geometric-mean TWAP oracle, which is both novel and empirically validated. The locked position mechanism provides cascade attenuation proportional to adoption, though its effectiveness depends on rational adoption dynamics that remain uncertain \cite[\S C]{apx}. The beta-distributed cap function rate-limits capacity accumulation but does not prevent long-term Sybil advantage. The conservative default LTV of 33\% protects solvency at the cost of capital efficiency, and is adjustable via governance. Formal verification and game-theoretic liquidation modeling remain the most significant gaps for future work.
...
\texttt{WEIGHT\_SUPPLY} & 85 & $\in$ & [0, 255] & Supply health weight \\
...
\texttt{SPREAD} & 10\% & $\in$ & [0, 50\%] & Rate half-spread \\
...
\texttt{CAP} & --- & $\in$ & [0, $2^{224}$] & Position cap \\
...
\midrule
\texttt{RAY} & $10^{27}$ & Ray precision (27 decimals) \\
...
\texttt{VERSION} & 0x1a5 & Protocol version \\
\bottomrule
\end{tabularx}
\end{table}
...
\newblock \emph{Companion Document}, 2026.
...
\noindent A comprehensive glossary with over 120 entries appears in \cite[Part~III]{apx}. The following terms are central to this paper.
...
\begin{description}[leftmargin=!,labelwidth=2.5cm]
    ...
    \item[Lethargic Governance] Parameter change model where values transition asymptotically toward targets, bounded to $0.5\times$--$2\times$ per governance cycle.

    \item[LTV] Loan-to-Value ratio --- maximum borrowing power as fraction of collateral value; default $w_s/w_b = 85/255 \approx 33\%$.

    \item[Position Lock] Irrevocable fraction of a position restricted from redemption; locked suppliers earn a bonus, locked borrowers receive a malus (discount), and locked positions act as circuit breakers during cascades.

    \item[Spread] (1)~Oracle: bidirectional geomean of relative spreads from both AMM query directions, stored as $\log_2(1 + s_{\text{geo}})$. (2)~IRM: symmetric half-spread applied to base rate; borrow rate = base~$\times$~$(1{+}s)$, supply rate = base~$\times$~$(1{-}s)$.

    \item[Supply / Borrow Position] ERC20 tokens representing collateral (standard transfer) and debt (inverted transfer: \texttt{transfer} \emph{pulls} debt). Health check on sender (supply) or receiver (borrow).

    \item[TWAP] Time-Weighted Average Price --- price smoothed via log-space EMA (geometric mean temporal averaging) to resist manipulation.

    \item[Utilization] Ratio of borrowed to supplied assets in a vault; drives interest rates via a piecewise-linear model with a kink at optimal utilization.
\end{description}
