...
Borrow rate = base * (1 + spread), Supply rate = util * base * (1 - spread).
The supply rate is scaled by utilization because interest is only earned on the
borrowed portion of the pool.
...
def supply_rate(
    ...
    """SupplyRate.by(): util * base_rate * (1 - spread). Rate.sol lines 114-118."""
    return rate_by(util, util_opt, rate_opt) * (1.0 - spread) * util
