...
The protocol maintains a symmetric spread around the gross rate $R(U)$: $R_{\text{borrow}} = R_{\text{base}} \cdot (1{+}s)$, while the supply rate is scaled by utilization, $R_{\text{supply}} = U \cdot R_{\text{base}} \cdot (1{-}s)$, because interest is only earned on the borrowed portion of the pool. This generates a $2s$ margin on the \emph{borrowed} interest flow (the spread surplus on borrowed principal $U \cdot S$), keeping the aggregate supply claim fully funded by borrower interest at every utilization level. See \cite{mtp} for mathematical foundations including interest accrual and time-weighted integration.
