...
\item \textbf{Permanent path} ($\texttt{dt\_term} = 2^{256}{-}1$): add to \texttt{perma} and \texttt{coded} total; no slot write, no bitmap change.
\end{enumerate}
...
\begin{tikzpicture}[
    ...
    % Perma arrow
    \draw[->, thick, dashed, codepurple] (-0.8,-1.8) node[left, font=\tiny] {perma} -- (0.8,-1.8) node[right, font=\tiny] {perma};
\end{tikzpicture}
...
\begin{align}
\texttt{spent}  &= \lfloor T \cdot t / Q \rfloor & &\text{(512-bit intermediate)} \label{eq:spent} \\
\texttt{fract}  &= T \cdot t \bmod Q & &\text{(sub-epoch remainder)} \label{eq:fract} \\
...
\label{lem:overflow}
Equations~\eqref{eq:spent}--\eqref{eq:result} compute $\Sigma Q - T t + pL$ exactly.
\end{lemma}
...
\begin{tikzpicture}[
    ...
    \draw[->, thick] (0,-0.4) -- (-0.75,-0.65) node[midway, right, font=\tiny] {$-$};
    \draw[->, thick] (0,-0.4) -- (1.25,-0.65) node[midway, left, font=\tiny] {$+$};
    % SLOADs
    \node[font=\tiny, codegray, anchor=west] at (3,0) {3 SLOADs};
    \node[font=\tiny, codegray, anchor=west] at (3,-0.4) {0 loops};
\end{tikzpicture}
...
\textbf{Delta from \S\ref{subsec:more}}: After the slot write, update \texttt{ls.depth[user]}:
\begin{itemize}[nosep]
...
\begin{proof}
By the \texttt{more()} precondition, $\texttt{dt\_term} \leq L = 15Q$. A lock created at time~$t$ targets epoch $e = \lfloor(t{+}\texttt{dt\_term})/Q\rfloor$. The current epoch is $e_{\text{now}} = \lfloor t/Q \rfloor$. Thus:
\[
e_{\text{now}} \leq e \leq \left\lfloor \frac{t + 15Q}{Q} \right\rfloor = \left\lfloor \frac{t}{Q} \right\rfloor + 15 = e_{\text{now}} + 15
\]
All active epochs lie in a window of at most 16 consecutive integers $[e_{\text{now}}, e_{\text{now}}{+}15]$. Two distinct integers in this window with the same value mod~16 must differ by exactly~16---but the window has width~$\leq 15$, a contradiction.
\end{proof}

\begin{corollary}[Cross-User Collision]
\label{cor:cross-user}
When \texttt{push(src, tgt)} copies a lock from source slot~$i$ to target slot~$i$, the target epoch must equal the source epoch (since both are active and share the same mod-16 index). The \texttt{\_setTarget} accumulate-path is always taken for active target slots.
\end{corollary}
...
Since $k \leq 16$ (ring-buffer width), even the worst-case \texttt{free()} or \texttt{push()} has a hard upper bound of 38~\texttt{SSTORE}s and 32~\texttt{SLOAD}s, ensuring gas predictability regardless of user behavior.
...
\item[Active Slot] A ring-buffer slot whose epoch $e_i \geq e_{\text{now}}$. Included in \texttt{stateAt} queries and not yet eligible for sweep.

\item[Bitmap] 16-bit mask tracking which slots contain active locks. Stored in the lower 16 bits of \texttt{coded}.

\item[Coded] Single storage word packing \texttt{[uint240 total | uint16 bits]}. Decoded via shift and mask (Equations~\ref{eq:decode-total}--\ref{eq:decode-bits}).

\item[Collision] Two distinct active epochs mapping to the same slot index ($e_i \bmod 16 = e_j \bmod 16$ with $e_i \neq e_j$). Proven impossible by Theorem~\ref{thm:collision}.
...
\item[Lock Ratio ($\lambda$)] Normalized commitment depth: $\texttt{depthOf}(u) / (\texttt{balanceOf}(u) \cdot L) \in [0,1]$.

\item[LOCK\_TERM ($Q$)] One quarter $\approx 91.3$ days---the epoch duration and ring-buffer granularity.

\item[LOCK\_TIME ($L$)] $15 \times Q \approx 45$ months---maximum lock duration and permanent lock depth cap.
...
\item[Position Layer] The \texttt{Position} contract that wraps the \texttt{Lock} library, enforcing the \texttt{free(tgt)} precondition and maintaining the global total.

\item[Ring Buffer] Circular array of 16 slots indexed by $e \bmod 16$. Overwrites stale entries automatically when epochs advance beyond the window.
...
\item[Slot] One of 16 ring buffer entries, indexed by $e \bmod 16$. Packs \texttt{uint32 epoch} and \texttt{uint224 value} in one storage word.
...
\item[Stale Slot] Slot with $e < e_{\text{now}}$---expired but not yet swept by \texttt{free()}. Causes \texttt{totalOf} inflation and \texttt{depthOf} deflation until cleared.
...
\item[Window] The 16-epoch span $[e_{\text{now}},\, e_{\text{now}}{+}15]$ of valid future slots. Any lock with $\texttt{dt\_term}$ exceeding the window is rejected, ensuring the collision-free invariant (Theorem~\ref{thm:collision}).
