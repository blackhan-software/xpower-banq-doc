...
% ==============================================================================
\banqAbstract{%
DeFi protocols benefit from knowing not just \emph{whether} tokens are locked but \emph{how long} they are committed. Naive implementations require $O(n)$ storage walks or unbounded gas. We present a 16-slot quarterly ring buffer that stores time-locked positions in constant per-slot storage with bitmap-guided $O(k)$ sweep ($k\leq 16$). The key contribution is an $O(1)$ cached depth metric: by storing the epoch-weighted sum $\Sigma = \sum v_i(e_i{+}1)$, the exact token-second integral $D = \Sigma Q - T t + p L$ can be reconstructed in two \texttt{SLOAD}s. The implementation returns the normalized form $\widehat{D} = \lfloor \Sigma Q/L \rfloor - \lfloor T t/L \rfloor + p$ (token-equivalent units, suitable for direct comparison with \texttt{balanceOf}) using two \texttt{Math.mulDiv} calls and no iteration. Each lock requires 1--3 \texttt{SSTORE}s; queries are $O(1)$ or $O(k)$; proportional transfer runs in $O(k)$. Storage is aggressively packed: per-user state occupies 10 storage words (8~ring + cache + depth), with the \texttt{cache} word holding three logical fields (\texttt{perma}, \texttt{total}, \texttt{bits}) in a single \texttt{SLOAD}. The mechanism is deployed in the XPower Banq lending protocol~\cite{wp}, where the lock depth drives graduated lock bonus/malus on interest rates. Measured gas: 66{,}688~warm / 179{,}860~cold for a timed \texttt{lockSupply}, 86{,}712~cold for a permanent lock.%
}{time-locked positions, ring buffer, token-seconds, DeFi lending, graduated commitment, Solidity}

% ==============================================================================
% 1. Introduction
% ==============================================================================
\banqPaper{Ring-Buffer Time Locks}{A 16-Slot Quarterly Ring Buffer with $O(1)$ Depth Tracking for Graduated Commitment in DeFi Lending}
...
\textbf{Ours} & $[0,16Q)$ & 10 words & $O(1)$ & $O(1)$ & Token$\cdot$s \\
\bottomrule
\end{tabular*}
\end{table}
...
The first 9 words (8~ring slots~+ \texttt{cache}) constitute the Ring-Lock layer. The \texttt{depth} mapping is the Time-Lock extension. The \texttt{cache} word merges the irrevocable \texttt{perma} amount, the cached ring \texttt{total}, and the active-slot \texttt{bits} bitmap into a single \texttt{SLOAD}/\texttt{SSTORE}---a key gas optimization, since most operations touch all three fields together. Per-call \texttt{amount} is bounded by \texttt{uint112}; \texttt{perma} saturates at $2^{120}{-}1$ (cumulative permanent deposits would need ${\sim}256$ max-sized adds before saturation).

\subsection{Bitmap Encoding}
\label{subsec:bitmap}

The \texttt{cache} word packs three values: the upper 120 bits store the irrevocable \texttt{perma} amount, the middle 120 bits store the cached ring \texttt{total}, and the lower 16 bits store the active-slot bitmap. Decoding:
\begin{align}
\texttt{perma} &= \texttt{cache} \gg 136 \label{eq:decode-perma} \\
\texttt{total} &= (\texttt{cache} \gg 16) \mathbin{\&} (2^{120}{-}1) \label{eq:decode-total} \\
\texttt{bits}  &= \texttt{cache} \mathbin{\&} \texttt{0xFFFF} \label{eq:decode-bits}
\end{align}
...
\item Require $\texttt{amount} \leq 2^{112}{-}1$ and $\texttt{dt\_term} > 0$. Revert otherwise.
\item If $\texttt{dt\_term} = 2^{256}{-}1$ (sentinel): execute \textbf{permanent path}---add to the \texttt{perma} field of \texttt{cache} (single \texttt{SSTORE}, since \texttt{perma}, \texttt{total}, and \texttt{bits} share one word); no slot write, no bitmap change. Return. \emph{(Early exit avoids overflow in the addition $t + \texttt{dt\_term}$ below.)}
...
\item Update \texttt{cache}: add \texttt{amount} to the \texttt{total} field, set bit~$i$ in \texttt{bits}.
\end{enumerate}

\textbf{Complexity}: 2~\texttt{SSTORE}s (slot~+ \texttt{cache}). Permanent: 1~\texttt{SSTORE} (\texttt{cache} only; \texttt{perma}~+ \texttt{total} share the word; cold-write gas dominates the first deposit). Measured: 179{,}860 / 66{,}688 gas (cold/warm) for timed \texttt{lockSupply}, 86{,}712 cold for permanent.
...
\item Read \texttt{cache} $\to$ extract \texttt{bits}. (\texttt{perma} and \texttt{total} are re-loaded just before the cache write-back.)
...
\item Write back \texttt{cache} with updated \texttt{total} and \texttt{bits} (\texttt{perma} preserved).
\end{enumerate}

\textbf{Complexity}: $k$~\texttt{SLOAD}s + up to $k$~\texttt{SSTORE}s (slots; only deleted/decremented slots) + up to 1~\texttt{SSTORE} (\texttt{cache}; skipped if neither \texttt{total} nor \texttt{bits} changed), where $k$ is the number of set bitmap bits. Measured worst-case (\texttt{free} 16 expired slots): 31{,}157~gas.
...
\item Batch-write both \texttt{cache} words (\texttt{perma}, \texttt{total}, \texttt{bits} together per side).
\end{enumerate}
...
\textbf{Complexity}: $(2k{+}2)$~\texttt{SLOAD}s ($2k$ slot reads + $2$ \texttt{cache} reads, one per side) + $2k$~\texttt{SSTORE}s + 2~\texttt{SSTORE}s (\texttt{cache}$\times$2; \texttt{perma} rides along in the same word). Measured \texttt{xfer\_supply}: 145{,}251 (perma only) / 191{,}463 (1~slot) / 402{,}525 (16~slots, worst case).
...
\textbf{Algorithm.}
\begin{enumerate}[leftmargin=*, nosep]
\item Read \texttt{cache} once; start with $\texttt{total} = \texttt{cache.perma}$.
\item Walk bitmap: for each active slot with $e_i \geq e_{\text{stamp}}$, add $v_i$ to total.
\item Return total.
\end{enumerate}

\textbf{Complexity}: 1~\texttt{SLOAD} (\texttt{cache}, supplies \texttt{perma} and \texttt{bits}; \texttt{total} is recomputed from active slots) + $k$~\texttt{SLOAD}s (bitmap-guided slot reads). No writes. Measured \texttt{lockStateAt} (16~slots view): 17{,}895~gas.

% ---------- 4.5 roll() ----------
\subsection{\texttt{roll(user, amount, dt\_term)} --- $O(k)$ Sweep-and-Re-Lock}
\label{subsec:roll}

\texttt{roll()} combines a partial sweep with a new lock in a single call: it drains up to \texttt{amount} tokens from the user's earlier active slots (those whose epoch is strictly before the target epoch) and re-locks the drained amount into the target slot. This is useful for extending an existing position's commitment without making separate \texttt{free()} and \texttt{more()} calls.

\textbf{Algorithm.}
\begin{enumerate}[leftmargin=*, nosep]
\item Compute target stamp $\texttt{stamp} = t + \texttt{dt\_term}$ and target epoch $e_{\text{tgt}} = \lfloor \texttt{stamp}/Q \rfloor$.
\item Walk the user's bitmap and, for each active slot with $e_i < e_{\text{tgt}}$, drain up to the remaining \texttt{amount} (using the same \texttt{\_freeAt} helper as \texttt{free()}).
\item Re-lock the drained total into the target slot via \texttt{\_more} (same code path as \texttt{more()}, including self-healing).
\end{enumerate}

\textbf{Complexity}: $O(k)$ --- one \texttt{\_freeAt} pass plus one \texttt{\_more} call. Same \texttt{dt\_term} semantics as \texttt{more()}.

% ---------- 4.6 totalOf() ----------
\subsection{\texttt{totalOf(user)} --- $O(1)$ Cached Query}
\label{subsec:totalof}

Decode the middle 120 bits of \texttt{cache}: a single \texttt{SLOAD}.
...
\textbf{Implementation form.} To match the token-equivalent units used by the bonus/malus layer (\S\ref{sec:integration}), the implementation returns the normalized quantity $\widehat{D} = D/L + p$ rather than $D$ itself. Both intermediate products are performed with OpenZeppelin's \texttt{Math.mulDiv}~\cite{oz-math}, which carries a 512-bit numerator natively so no separate sub-epoch remainder term is needed:
\begin{align}
\texttt{gross}    &= \lfloor \Sigma \cdot Q / L \rfloor                 \label{eq:gross} \\
\texttt{spent}    &= \lfloor \mathit{total} \cdot t / L \rfloor          \label{eq:spent} \\
\texttt{depthOf}(u) &= \texttt{gross} - \texttt{spent} + p              \label{eq:result}
\end{align}
where \texttt{total} (the timed total stored in \texttt{cache}, distinct from \texttt{perma}) is exactly $T$ from~\eqref{eq:timed}.

\begin{lemma}[Overflow-Safe Computation]
\label{lem:overflow}
Equations~\eqref{eq:gross}--\eqref{eq:result} compute $\lfloor \Sigma Q/L \rfloor - \lfloor T t/L \rfloor + p$ exactly via two independent 512-bit \texttt{mulDiv} operations.
\end{lemma}

\begin{proof}
\texttt{Math.mulDiv}$(a,b,c)$ returns $\lfloor a\cdot b / c \rfloor$ for any 256-bit $a,b,c$ without intermediate overflow, because the product $a\cdot b$ is held in a 512-bit pair before the final division~\cite{oz-math}. Applied to $(\Sigma, Q, L)$ and $(\mathit{total}, t, L)$, this directly yields the two floors above. The closing addition of the 120-bit \texttt{perma} field cannot exceed $2^{256}$.
\end{proof}

\begin{lemma}[$\Sigma$ Overflow Safety]
\label{lem:sigma-overflow}
Let $S$ denote the maximum per-user locked total and $E$ the current epoch. If $S \cdot (E{+}16) < 2^{256}$, then the stored $\Sigma$ fits in \texttt{uint256}. The intermediate product $\Sigma \cdot Q$ in~\eqref{eq:gross} is then carried by \texttt{Math.mulDiv} with full 512-bit precision and never materializes as a single 256-bit value.
\end{lemma}

\begin{proof}
All active epochs lie in $[E, E{+}15]$ (Theorem~\ref{thm:collision}), so $(e_i{+}1) \leq E{+}16$. The sum of slot values satisfies $T = \sum v_i \leq S$. Thus $\Sigma = \sum v_i(e_i{+}1) \leq S(E{+}16) < 2^{256}$. The subsequent product $\Sigma\cdot Q$ in \texttt{mulDiv} is held in a 512-bit register before the division by $L$, so no further overflow check is needed.

\emph{Practical margin.} The slot epoch field is a \texttt{uint16} (\S\ref{subsec:storage-layout}), so $E < 2^{16}$. With $E{+}16 < 2^{16}$ this leaves $S < 2^{240}$---far above any realistic ERC-20 token supply. The Position layer's \texttt{cap()} mechanism provides the enforcement point.
\end{proof}
...
\textbf{Complexity}: 3~\texttt{SSTORE}s (slot + \texttt{cache} + \texttt{depth}). Permanent: 1~\texttt{SSTORE} (same as Ring-Lock---\texttt{depth} unchanged for permanent locks).
...
\textbf{Complexity}: $k$~\texttt{SSTORE}s (slots) + 2~\texttt{SSTORE}s (\texttt{cache} + \texttt{depth}).
...
\textbf{Complexity}: $2k$~\texttt{SSTORE}s + 4~\texttt{SSTORE}s (\texttt{cache}$\times$2, \texttt{depth}$\times$2; \texttt{perma} packed in \texttt{cache}).
...
\textbf{Units note}: The accumulator above is in raw token-seconds, but \texttt{stateAt} applies the same $/L$ normalization as \texttt{depthOf} (\S\ref{subsec:depthof}, eq.\ on p.\,\pageref{subsec:depthof}) before returning, so the reported \texttt{depth} is in token-equivalent units (committed tokens remaining), not token-seconds.
...
This is the key operation.  Rather than looping over every active slot, \texttt{depthOf()} reconstructs the token-seconds value from three cached storage words ($\Sigma$, \texttt{total}, $p$) and the current timestamp, using the Depth Identity.  To match the units used by the bonus/malus layer (\S\ref{sec:integration}), the function returns the normalized form
\[
\widehat{D}(u,t) \;=\; \bigl\lfloor \Sigma \cdot Q / L \bigr\rfloor \;-\; \bigl\lfloor \mathit{total}\cdot t / L \bigr\rfloor \;+\; p
\]
i.e.\ $D/L + p$ expressed in token units (with \texttt{mulDiv} performing each division with full 512-bit intermediate precision so no overflow can occur). The result is a single $O(1)$ read with no loops and no bitmap traversal.
...
Note that \texttt{cache} stores \texttt{perma} and \texttt{total} as \emph{independent} fields (\texttt{total} is the timed total; permanent locks are not part of it), so no $\mathit{total}-\mathit{perma}$ subtraction is needed. The single \texttt{Math.mulDiv} call replaces the older \texttt{mulDiv}/\texttt{mulmod} decomposition with a 512-bit native pathway.

\textbf{Complexity}: 2~\texttt{SLOAD}s (\texttt{cache} supplies \texttt{perma}~+ \texttt{total}; \texttt{depth} loaded separately). No loops, no bitmap iteration.
...
\textbf{Idealized depth} (token-seconds, for intuition):
\begin{align*}
D &= \Sigma Q - Tt + pL \\
...
\textbf{Via \texttt{depthOf}} ($O(1)$, normalized form returned by the implementation, $L = 16Q$):
\begin{align*}
...
\textbf{Via \texttt{stateAt}} ($O(k)$, verification of the idealized $D$):
\begin{align*}
...
When \texttt{push(src, tgt)} copies a lock from source slot~$i$ to target slot~$i$, and the target already has an active lock at index~$i$, then the target epoch must equal the source epoch (since both are active and share the same mod-16 index). The target-side accumulate-path of \texttt{\_pushSlot} is therefore always taken for active target slots; empty or swept target slots take the overwrite path.
\end{corollary}
...
\begin{proof}
\texttt{totalOf} returns the \texttt{total} field from \texttt{cache}, which includes contributions from all slots with set bitmap bits---both active and expired. \texttt{stateAt} sums only slots with $e_i \geq e_{\text{now}}$. Since every active slot is counted by both, and expired slots are counted only by \texttt{totalOf}:
\begin{align*}
...
\begin{proof}
The stale target slot has $(e_{\text{stale}}, v_{\text{stale}})$ with $e_{\text{stale}} < e_{\text{now}}$. When the target-side branch of \texttt{\_pushSlot} finds $e_{\text{slot}} \neq e_{\text{incoming}}$, it overwrites: $v \leftarrow \Delta v$, $e \leftarrow e_{\text{incoming}}$. The overwritten $v_{\text{stale}}$ was still counted in \texttt{cache[tgt].total} and \texttt{depth[tgt]}, but is now lost---\texttt{free()} will see the new active epoch and skip the slot.
\end{proof}
...
\item \textbf{\texttt{push()} source}: the source-side branch of \texttt{\_pushSlot} returns $\lnot\texttt{mask}(i)$ (clearing bit) only when $v = 0$ after subtraction.
\item \textbf{\texttt{push()} target}: the target-side branch of \texttt{\_pushSlot} returns $\texttt{mask}(i)$ (setting bit), and the loop skips $\Delta v = 0$.
\end{itemize}
...
\begin{proof}
In \texttt{more()} with $\texttt{dt\_term} = 2^{256}{-}1$: only the \texttt{cache} word is updated (the \texttt{perma} field, alongside the unchanged \texttt{total} and \texttt{bits}); \texttt{depth} is untouched. In \texttt{free()}: only timed slots are swept. In \texttt{push()}: the \texttt{perma} block of \texttt{\_push} transfers between \texttt{cache[src].perma} and \texttt{cache[tgt].perma} without touching \texttt{depth}. The \texttt{depth} register exclusively tracks $\sum v_i(e_i{+}1)$ for timed slots.
\end{proof}
...
\texttt{more} (perma) & 1 \texttt{SSTORE} & 1 \texttt{SSTORE} & $0$ \\
\texttt{free} & $k{+}1$ \texttt{SSTORE} & $k{+}2$ \texttt{SSTORE} & $+1$ \\
\texttt{push} & $2k{+}2$ \texttt{SSTORE} & $2k{+}4$ \texttt{SSTORE} & $+2$ \\
\texttt{totalOf} & 1 \texttt{SLOAD} & 1 \texttt{SLOAD} & $0$ \\
\texttt{depthOf} & --- & 2 \texttt{SLOAD} & new \\
...
Since $k \leq 16$ (ring-buffer width), even the worst-case \texttt{push()} has a hard upper bound of 36~\texttt{SSTORE}s and 36~\texttt{SLOAD}s ($2k$ slots $+$ 4 ancillary: \texttt{cache}$\times$2, \texttt{depth}$\times$2; \texttt{perma} packed inside \texttt{cache}), ensuring gas predictability regardless of user behavior. Empirically, \texttt{xfer\_supply} measures 145{,}251~gas (perma only) up to 402{,}525~gas (16 active slots, worst case).
...
\]
where $\lambda \in [0,1]$ measures normalized commitment depth (since \texttt{depthOf} is already normalized to token units by division through $L$, see \S\ref{subsec:depthof}).
\end{definition}
...
\text{Supply spread:} \quad s_{\text{eff}} &= s_0 - \min\!\bigl(s_0,\;\beta_s \cdot \overline{\lambda}(u)\bigr) \label{eq:supply-bonus} \\
\text{Borrow spread:} \quad s_{\text{eff}} &= s_0 - \min\!\bigl(s_0,\;\beta_b \cdot \overline{\lambda}(u)\bigr) \label{eq:borrow-malus}
\end{align}
where $\lambda_{\text{snap}}$ is the user's lock ratio at the last snapshot, $\beta_s = \texttt{LOCK\_BONUS}$ and $\beta_b = \texttt{LOCK\_MALUS}$ are governance parameters, and $s_0$ is the model's base spread. The supply and borrow rates are then derived from the kinked IR model with the reduced spread; see~\cite{wp}~\S4.7 for the full rate composition.
...
\begin{enumerate}[leftmargin=*, nosep]
\item A 16-slot quarterly ring buffer providing bounded storage per user (10 words).
