...
With spread $s = 10\%$, $r_{\text{bonus}} = r_{\text{malus}} = s$, and utilization $U$. The supply rate is scaled by utilization (interest is only earned on the borrowed portion of the pool), so supplier multipliers carry an explicit $U$; borrower multipliers do not:
...
\midrule
Unlocked Supplier & $U(1-s)$ & $U(1-s)-1$ \\
Locked Supplier & $U(1+r_{\text{bonus}})(1-s)$ & $U(1+r_{\text{bonus}})(1-s)-1$ \\
Unlocked Borrower & $(1+s)$ & $+s$ \\
Locked Borrower & $(1-r_{\text{malus}})(1+s)$ & $(1-r_{\text{malus}})(1+s)-1$ \\
\bottomrule
\end{tabularx}
\end{center}

At $U = 100\%$ the supplier multipliers reduce to the familiar $(1-s) = 0.90$ and $(1+r_{\text{bonus}})(1-s) = 0.99$.

The \textbf{APY differential} between locked and unlocked positions (supplier side):
\begin{equation}
\Delta r = U \times r_{\text{base}} \times (1-s) \times r_{\text{bonus}} = U \times r_{\text{base}} \times 0.9 \times 0.1 = 0.09 \times U \times r_{\text{base}}
\end{equation}
...
The base interest rate $r_{\text{base}}$ varies with utilization $U$ according to the piecewise-linear model (\S4.7 of~\cite{wp}), and the supply-side APY differential $\Delta r = 0.09 \times U \times r_{\text{base}}$ carries the utilization factor $U$. Table~\ref{tab:nash-breakeven} presents breakeven periods for various utilization levels.
...
\midrule
80\% & 8.9\% & 0.64\% & 7.8y & 4.7y \\
90\% & 10\% & 0.81\% & 6.2y & 3.7y \\
95\% & 55\% & 4.70\% & 13m & 8m \\
98\% & 82\% & 7.23\% & 8m & 5m \\
100\% & 100\% & 9.0\% & 6.7m & 4m \\
\bottomrule
...
Protocol margin per unit interest flow (on the \emph{borrowed} flow: the supply side is utilization-scaled, so the absolute surplus is $U \cdot S \cdot r_{\text{base}} \cdot M(\bar{\rho})$):
\begin{equation}
