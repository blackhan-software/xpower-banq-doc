...
% ==============================================================================
\banqAbstract{%
Compounding interest indices in DeFi lending protocols grow exponentially and eventually overflow fixed-width integer storage. The standard multiplicative index --- a running product of $\exp(r_i)$ factors stored in \texttt{uint256} at RAY precision (${10^{27}}$) --- exhausts its 115.3 e-folds of headroom in as few as 29~years under worst-case rate assumptions. We present a \emph{log-space compounding index} that replaces the stored product with its logarithm: accrual becomes addition ($L \leftarrow L + r$), and the growth factor is reconstructed on demand via $\exp(L - L_u)$. The transformation eliminates overflow entirely (linear accumulation: \mbox{$\sim 5 \times 10^5$--$10^{58}$} years depending on the chosen field width --- many orders of magnitude beyond the $\sim 29$~year horizon of the multiplicative form), replaces write-path exponentiation with addition, and defers \texttt{exp()} to individual read paths. Counter-intuitively, it \emph{improves} numerical precision by avoiding the compounded truncation bias inherent in iterated fixed-point multiplication. We provide the mathematical foundation, a complete Solidity code transformation, gas benchmarks across 23~pool operations ($-114{,}447$~gas net, $-0.40\%$), adversarial analysis, and empirical precision measurements confirming the log-space form is closer to analytical values than the multiplicative form it replaces. The mechanism is deployed in the XPower Banq lending protocol~\cite{wp}.%
}{compounding index, overflow, log-space, fixed-point arithmetic, DeFi lending, interest rate, Solidity}

% ==============================================================================
% 1. Introduction
% ==============================================================================
\banqPaper{Log-Space Compounding Index}{An Overflow-Free Interest Index via Additive Log-Space Accumulation for DeFi Lending}
...
This design, while mathematically clean, stores the cumulative product of exponentials in a fixed-width integer. In Solidity's \texttt{uint256} at RAY precision ($10^{27}$), the index has approximately 115.3 e-folds of growth before overflow. At the XPower Banq protocol's maximum rate of 200\% (the \texttt{Rate.by} cap~\cite{wp}), this budget is exhausted in ${\sim}57.7$~years; with borrow-side spread at 100\% utilization, in ${\sim}28.9$~years.
...
\textbf{Aave Liquidity Index.} Aave~\cite{aave2020} uses a RAY-precision ($10^{27}$) liquidity index with the same multiplicative structure: $I_{n+1} = I_n \times (1 + r \cdot \Delta t / \text{YEAR})$. The RAY scale provides 62.2 e-folds of precision overhead, leaving 115.3 e-folds for growth --- the same budget analysed in this paper.
...
The 200\% cap originates from \texttt{Rate.by()}, which clamps output at \texttt{Constant.TWO} ($2 \times 10^{18}$). The borrow-side spread multiplies this further: $r_{\text{borrow}} = r_{\text{base}} \times (1 + s)$, where $s$ is the spread parameter. The supply side does not add overflow pressure: it is utilization-scaled ($r_{\text{supply}} = U \times r_{\text{base}} \times (1 - s)$, so at most $200\%$), never exceeding the base-rate cap.
...
\subsection{Shipped Storage Layout}
\label{subsec:shipped-layout}

The conceptual \texttt{uint256 \_index\_wad} and \texttt{mapping(address \texttt{=>} uint256) \_userIndex} of Listing~\ref{lst:log-storage} are not allocated as separate storage in production. The shipped \texttt{Position.sol} packs the global index into the existing \texttt{\_state} word and the per-user index into the \texttt{\_stateOf[user]} word, alongside fields that would otherwise occupy independent slots:

\begin{lstlisting}[language=Solidity, caption={Packed Position storage (Position.sol:511--608)}, label={lst:packed-state}]
// Global state (1 word):
...
\begin{itemize}[leftmargin=*, nosep]
\item \texttt{index\_log}: 80~bits, WAD scale $\to$ ${\sim}1.7 \times 10^6$~years at 100\% APR (per-user) and ${\sim}5 \times 10^5$~years at 1000\% APR (global) before saturation. Theorem~\ref{thm:overflow}'s overflow-freedom property holds within this finite budget; the log-space form simply makes the budget unreachable in practice (cf.\ Remark~\ref{rem:modular}).
\item \texttt{stamp}: 64~bits, seconds $\to$ $5.8 \times 10^{11}$~years (42$\times$ the age of the universe).
\item \texttt{depth\_wad}: 64~bits, dimensionless WAD ratio (\texttt{lock.depthOf(user)} / \texttt{balanceOf(user)}, cf.\ \texttt{Position.sol:488--494}) $\to$ 18$\times$ the $[0, 10^{18}]$ WAD range.
\item \texttt{money}: 112~bits, token units $\to$ $5 \times 10^{15}$ tokens at 18~decimals.
\end{itemize}

This packing is the source of much of the gas saving documented in \S\ref{sec:gas}: every \texttt{\_indexOf} write that would have been a separate \texttt{SSTORE} on \texttt{\_index\_wad} becomes part of the same word as \texttt{stamp} and \texttt{large\_holders}, and every per-user accrual snapshot writes \texttt{index\_log} alongside \texttt{depth\_wad} and \texttt{money} in a single \texttt{SSTORE}. The \texttt{LOG\_INDEX.md} spec captures the same observation: ``the shipped contract instead packs the global index into the \texttt{\_state} word (\texttt{uint80 index\_log}) and the per-user index into the \texttt{\_stateOf} word (also \texttt{uint80}).'' All mathematical content of \S\S\ref{subsec:formal}--\ref{subsec:snapshots} applies unchanged; only the storage names differ. The lock-yield helper of \S\ref{subsec:lockyield} has been further restructured in the shipped contract --- see Remark~\ref{rem:lockyield-shipped}.
...
\label{thm:overflow}
Treated as a full \texttt{uint256}, the log-space index $L(t)$ cannot overflow within $2.9 \times 10^{58}$~years at 400\%/year. In the shipped storage, where \texttt{index\_log} occupies the upper 80~bits of \texttt{\_state} (cf.\ \S\ref{subsec:shipped-layout}), the index saturates after $\sim 5 \times 10^5$~years at 1000\%/year (global) and $\sim 1.7 \times 10^6$~years at 100\%/year (per-user) --- both unreachable in practice and dwarfing the $\sim 29$~year horizon of the multiplicative RAY index.
\end{theorem}
...
\label{prop:snapshot}
$L_u$ is set to $L(t)$ during \texttt{\_snapUser} (\texttt{Position.sol:488--498}). The difference $L(t) - L_u$ accumulates only yields since $u$'s last state transition.
\end{property}
...
\begin{remark}[Wrap behaviour]
\label{rem:modular}
In the shipped contract, the global \texttt{index\_log} field occupies the upper 80 bits of \texttt{\_state} and is masked, not modular-added, on every write (\texttt{Position.sol:586--598}). The read-time delta \texttt{index\_log - user\_index} is \emph{checked} and guarded by \texttt{index\_log > user\_index} (\texttt{Position.sol:123}); after a hypothetical wrap, the guard fails and the function returns the user's principal without accrued interest, rather than recovering $\Delta L$ via modular subtraction. The Uniswap~v3 tick accumulator pattern~\cite{uniswap2021} (which does use wrap-aware subtraction) is therefore an analogue of the \emph{additive} structure, not of the shipped wrap behaviour. The 80-bit budget makes wrap unreachable (${\sim}5 \times 10^5$~years at 1000\%/yr; cf.\ \S\ref{subsec:shipped-layout}), so this is a documentation point, not a soundness issue.
\end{remark}
...
\begin{remark}[Shipped lock-yield path]
\label{rem:lockyield-shipped}
The shipped \texttt{Position.sol} does not expose \texttt{\_lockYieldOf} as a separate helper. The bonus/malus is instead applied through a per-user spread differential (\texttt{\_spreadDiff} / \texttt{\_model(user,~rate\_param\_id)}) that lowers the effective spread in \texttt{\_indexOf} for users with non-zero lock depth (see \texttt{Position.sol:84--103, 657--660, 738--741}). The yield-fraction identity $\exp(\Delta L) - 1$ presented above is preserved mathematically: it is now absorbed into the additive log-increment rather than computed as a separate \texttt{growth - 1} term at read time.
\end{remark}
...
\begin{enumerate}[leftmargin=*, nosep]
\item \textbf{Overflow elimination}: linear accumulation provides $\sim 5 \times 10^5$~years of headroom at 1000\%/yr in the shipped 80-bit field, $\sim 10^{58}$~years in the full \texttt{uint256}, vs.\ ${\sim}29$~years for the multiplicative form.
