---
title: "Appendix C: Lock Incentive Analysis"
description: "Nash equilibrium analysis for lock adoption, including NPV analysis, breakeven periods, and protocol margin."
---

# Appendix C: Lock Incentive Analysis
...
With spread $s = 10\%$ and $r_{\text{bonus}} = r_{\text{malus}} = s\text{:}$
...
For a supplier with lock ratio $\rho \in [0,1]$ and holding period $T\text{:}$
...
Lock adoption is rational when $U_{\text{locked}} > U_{\text{unlocked}}\text{:}$

$$(1 + r_{\text{locked}})^T \times (1 - D) > (1 + r_{\text{unlocked}})^T$$

Solving for the **breakeven holding period** $T^*\text{:}$

$$T^* = \frac{\ln(1-D)}{\ln(1 + r_{\text{unlocked}}) - \ln(1 + r_{\text{locked}})} \approx \frac{D}{\Delta r}$$

## Breakeven Analysis by Utilization

The base interest rate $r_{\text{base}}$ varies with utilization $U$ according to the piecewise-linear model (Whitepaper, Section 4.7). The following table presents breakeven periods for various utilization levels.
...
Locked positions gain *de facto* seniority in liquidations because liquidators prefer unlocked positions (immediate liquidity). The seniority value $S(\bar{\rho})$ depends on aggregate lock adoption $\bar{\rho}$ (denoted $\phi$ in the Cascade Attenuation Theorem, Whitepaper Section 4.1):
...
With $r_{\text{bonus}} = r_{\text{malus}} = s$ and assuming $\bar{\rho}^S = \bar{\rho}^B = \bar{\rho}\text{:}$
...
::: theorem
**Theorem 5** (Lock Adoption Equilibrium). *With $r_{\text{bonus}} = r_{\text{malus}} = s = 10\%\text{:}$*
...
Together, these properties confirm Theorem 5: the mechanism is self-regulating, with per-parameter solvency bounds that require no on-chain adoption tracking, cascade dynamics that naturally throttle liquidation velocity, and welfare effects that the interest rate curve absorbs without instability.
...
within the sustainable range of the protocol margin table above.

### Cascade Dynamics and Keeper Sizing

Debt assumption transfers locks proportionally, preserving aggregate $\bar{\rho}$ through cascades—the attenuation bound $(1{-}\phi)$ from the Whitepaper (Theorem 4.1) holds dynamically, not merely at a pre-crash snapshot. At $\bar{\rho} \approx 30\%$, this reduces 25%-shock liquidations from 85.7% to approximately 55% of positions (Whitepaper, Table 5). However, locked supply received by a liquidator cannot be redeemed: absorbing a fully-locked victim at exponent $e$ increases assumed debt by $d = V_b \cdot 2^{-e}$ while adding zero redeemable supply, eroding headroom for subsequent liquidations.
