...
::: proof
By the `more()` precondition, $\texttt{dt\_term} \leq L = 15Q$. A lock created at time $t$ targets epoch $e = \lfloor(t{+}\texttt{dt\_term})/Q\rfloor$. The current epoch is $e_{\text{now}} = \lfloor t/Q \rfloor$. Thus:

$$e_{\text{now}} \leq e \leq \left\lfloor \frac{t + 15Q}{Q} \right\rfloor = \left\lfloor \frac{t}{Q} \right\rfloor + 15 = e_{\text{now}} + 15$$

All active epochs lie in a window of at most 16 consecutive integers $[e_{\text{now}}, e_{\text{now}}{+}15]$. Two distinct integers in this window with the same value mod 16 must differ by exactly 16—but the window has width $\leq 15$, a contradiction. $\blacksquare$
:::

::: theorem
**Corollary 6.2** (Cross-User Collision). When `push(src, tgt)` copies a lock from source slot $i$ to target slot $i$, the target epoch must equal the source epoch (since both are active and share the same mod-16 index). The `_setTarget` accumulate-path is always taken for active target slots.
:::

::: proof
Follows directly from Theorem 6.1 applied to the target user's active set augmented with the incoming epoch. $\blacksquare$
:::
...
::: proof
By Theorem 5.1, $D = \Sigma Q - Tt + pL$. After `free()`, the expired set $E = \emptyset$, so the stored $\Sigma$ and $T$ exactly represent the active set $A$. The identity reconstructs $D$ identically to the sum in the token-seconds definition. $\blacksquare$
:::
...
::: proof
`totalOf` returns the `total` from `coded`, which includes contributions from all slots with set bitmap bits—both active and expired. `stateAt` sums only slots with $e_i \geq e_{\text{now}}$. Since every active slot is counted by both, and expired slots are counted only by `totalOf`:
...
::: proof
`free()` deletes every slot with $e_i < e_{\text{now}}$, making $E = \emptyset$. The expired-set contributions in Theorems 6.4 and 6.5 vanish. $\blacksquare$
:::
...
::: proof
The stale target slot has $(e_{\text{stale}}, v_{\text{stale}})$ with $e_{\text{stale}} < e_{\text{now}}$. When `_setTarget` finds $e_{\text{slot}} \neq e_{\text{incoming}}$, it overwrites: $v \leftarrow \Delta v$, $e \leftarrow e_{\text{incoming}}$. The overwritten $v_{\text{stale}}$ was still counted in `coded[tgt]` and `depth[tgt]`, but is now lost—`free()` will see the new active epoch and skip the slot. $\blacksquare$
:::
...
::: proof
**For `stateAt`** ($t \geq t_{\text{now}}$): The effective time is $t_{\text{eff}} = \max(t, t_{\text{now}}) = t$. Each included slot satisfies $e_i \geq e_{\text{stamp}} = \lfloor t/Q \rfloor$, so $(e_i{+}1)Q \geq (\lfloor t/Q \rfloor {+} 1)Q > t$. Each term $v_i((e_i{+}1)Q - t) > 0$. The permanent contribution $pL \geq 0$.
...
- **`push()` source**: `_setSource` returns $\lnot\texttt{mask}(i)$ (clearing bit) only when $v = 0$ after subtraction.
- **`push()` target**: `_setTarget` returns $\texttt{mask}(i)$ (setting bit), and the loop skips $\Delta v = 0$.
...
::: proof
In `more()` with $\texttt{dt\_term} = 2^{256}{-}1$: only `perma` and `coded` are updated; `depth` is untouched. In `free()`: only timed slots are swept. In `push()`: `_movPerma` transfers between `perma[src]` and `perma[tgt]` without touching `depth`. The `depth` register exclusively tracks $\sum v_i(e_i{+}1)$ for timed slots. $\blacksquare$
:::
